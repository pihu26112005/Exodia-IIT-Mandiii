    ...
    <>
      <div className="relative w-full h-screen bg-black text-yellow-400 overflow-x-hidden ">
        ...
        <div className='h-screen bg-black'>
          ...
          <div className={` ${aboutNPfont11.className} relative text-center  pt-[13rem] max-sm:pt-[12rem] max-lg:pt-[12rem] text-6xl text-black font-semibold max-sm:text-black max-lg:text-5xl max-sm:text-3xl bgProp`}>
            TIMELINE
          </div>
          <div className='relative  font-bold flex justify-center mt-10 gap-40 text-4xl max-lg:text-3xl text-black max-lg:gap-20 max-sm:gap-10 max-sm:text-xl'>
            <div
          ...
          {/* @ts-ignore */}
          <div className='relative   text-black flex justify-center '><ScrollTimeline selectedDay={selectedDay} /> </div>
        </div>
      ...
      {active ? <div className="fixed cursor-pointer  top-48 left-0 right-0 z-10 flex flex-col items-center space-y-10 max-sm:space-y-4 max-sm:top-28 max-sm:text-xl font-harryPotter  text-3xl text-white">
        ...
        <div className="max-w-full mx-0 px-0 max-sm:hidden">
          <div className={`p-4 flex justify-center items-center text-[4rem] mt-8 ${aboutNPfont11.className}`}>TimeLine of All Events</div>
          <HoverEffect items={ScheduleEventList} className={`${aboutNPfont11.className}`} />
        </div>
